...
def simulate_robot(
    warehouse: Warehouse, robot: tuple[int, int], movements: str
) -> Warehouse:
...
def move_robot(
    warehouse: Warehouse, robot: tuple[int, int], move: str
) -> tuple[int, int]:
...
def find_move_target(
    warehouse: Warehouse, robot: tuple[int, int], direction: Direction
) -> tuple[int, int] | None:
...
def push_layer(
    warehouse: Warehouse, layer: list[tuple[int, int]], dy: int
...
def at_pos(warehouse: Warehouse, position: tuple[int, int]) -> Tile | None:
    return (
        warehouse[position[1]][position[0]] if is_inside(warehouse, position) else None
    )
...
def main():
    ...
    warehouse_map, robot, movements = list(parse_input(input_text.splitlines()))
